...
! integration over the angular coordinates for matrix elements with 
! symmetry-adapted subshell states. This includes the definition and 
! set-up of the reduced coefficients of fractional parentage as well as
! of reduced matrix elements for the tensor operators W^(k_q k_j) and
! T^{(k). Many of these coefficients are given by tables as defined in 
! the header of this module. Currently, coefficients are provided for
! subshells with j <= 9/2 in jj-coupling.
!-----------------------------------------------------------------------
   !
   use rabs_angular
   use rabs_constant
   ...
   public  :: rcfp_get_coefficient
                 ! Returns the value of the reduced coefficient of fractional 
                 ! parentage (rcfp) for subshells with j = 1/2, 3/2, 5/2, 7/2.
   public  :: rcfp_get_reduced_W
                 ! Returns the value of the reduced matrix elements W^(k_q k_j)
		 ! for subshells with j = 1/2, 3/2, 5/2, 7/2, 9/2. 
   private :: rcfp_get_reduced_W_from_table
                 ! Returns the value of the reduced matrix elements W^(k_q k_j)
		 ! for subshells with j = 1/2, 3/2, 5/2, 7/2 by looking up 
                 ! appropriate tables.
   private :: rcfp_get_reduced_W_all
                 ! Auxiliarity routine for rcfp_get_reduced_W. Returns the
                 ! values of the reduced matrix elements of W^(00), W^(10),
                 ! and W^(01) for j= 1/2, 3/2, 5/2, 7/2 from tables.
   private :: rcfp_get_reduced_W_three_half 
                 ! Auxiliary  routine for rcfp_get_reduced_W. Returns the
                 ! the value of the reduced matrix elements of W^(12) and 
                 ! W^(03) for j= 3/2 from tables.
   private :: rcfp_get_reduced_W_five_half
                 ! Auxiliary routine for rcfp_get_reduced_W. Returns the
                 ! value of the reduced matrix elements of W^(12), W^(03), 
                 ! W^(14), and W^(05) for j= 5/2 from tables.   
   private :: rcfp_get_reduced_W_seven_half
                 ! Auxiliary routine for rcfp_get_reduced_W. Returns the
                 ! value of the reduced matrix elements of W^(12), W^(03),  
                 ! W^(14), W^(05), W^(16), and W^(07) for j= 7/2 from tables.
   public  :: rcfp_calculate_Wk_me
                 ! Calculate the value of the matrix element W^(k_j) 
		 ! subshells with j = 1/2, 3/2, 5/2, 7/2, 9/2. 
   public  :: rcfp_calculate_Wk_times_Wk_0_me
                 ! Calculate the value of the matrix element 
		 ! [ W^(k_j) * W^(k_j) ]^(0) for subshells with 
                 ! j= 1/2, 3/2, 5/2, 7/2, 9/2.
   public  :: rcfp_calculate_a_times_Wk_me
                 ! Calculates the value of the matrix element 
		 ! [ a^(q j)_m_q * W^(k_j) ]^(0) for subshells with 
		 ! j = 1/2, 3/2, 5/2, 7/2, 9/2.
   public  :: rcfp_calculate_Wk_times_a_me
                 ! Calculates the value of the matrix element 
		 ! [ W^(k_j) * a^(q j)_m_q ]^(0) for subshells with  
		 ! j = 1/2, 3/2, 5/2, 7/2, 9/2.
   public  :: rcfp_calculate_a_me
                 ! Calculates the value of the matrix element 
                 ! (j^N QJ :: a^(q j)_m_q :: j^N' QJ) for subshells with
                 ! j= 1/2, 3/2, 5/2, 7/2, 9/2. 
   public  :: rcfp_Q_space_delta
                 ! Calculate trivial delta factors in the Q space.
   public  :: rcfp_input
                 ! Collects and proceeds all input for the calculations of 
                 ! coefficients of fractional parentage (cfp and rcfp) and
                 ! further reduced matrix elements for symmetry-adapted
                 ! subshell states. 
   private :: rcfp_analyse
                 ! Analyzes a given input string for different angular momenta
