...
!***** July 2014 *****
!-----------------------------------------------------------------------
! This module processes and maintains all informations concerning the 
! present Configuration Symmetry List and the representation of a number
! of atomic states in a CSF basis. It enables reading of such a list 
! from a given file and set all necessary arrays. 
! Several abstract data types are defined to "keep" all data of a 
! single configuration state function as well as of a CSF and ASF basis
! simply together. It also defines an 'atomic basis' in terms of a 
! determinant representation which might be useful in different 
! applications.
!-----------------------------------------------------------------------
   !
   use rabs_constant
   ...
   public  :: add_csf_to_basis
                 ! Appends all CSF with given symmetries from an 'old' to a
		 ! 'new' configuration scheme. 
   public  :: combine_configuration_scheme
                 ! Combines two configuration schemes into a third one due to
                 ! given cases.
   public  :: copy_basis_to_basis
                 ! Copies an 'old' configuration scheme into a 'new' one.
   public  :: copy_csf_to_csf
                 ! Copies a CSF from an 'old' configuration scheme as
		 ! CSF of a 'new' one.
   public  :: deallocate_asf_basis
                 ! Deallocates the storage of type(asf_basis) including
                 ! the allocation of all substructures.
   public  :: deallocate_asf_det_basis
                 ! Deallocates the storage of type(asf_det_basis) including
                 ! the allocation of all substructures.
   public  :: deallocate_csf_basis
                 ! Deallocates the storage of type(csf_basis) including
                 ! the allocation of all substructures.
   public  :: get_subshell_seniority
                 ! Returns the seniority quantum number for a given
                 ! antisymmetric subshell state.
   public  :: load_csl_from_file
                 ! Reads in an parses the configuration symmetry list.
   public  :: load_mix_file_grasp2k
                 ! Loads a GRASP92 .mix file into an appropriate data structure.
   private :: parse_intermediate_X
                 ! Parses the intermediate and total angular momenta 
                 ! and parity for a given CSF.
   ...
   public  :: set_configuration_scheme
                 ! Creates a new 'configuration scheme' by coupling an
		 ! additional electron to previously defined set of CSF.
   !
   ...
   type, public :: as_hfs_function
      ...
      integer(kind=i1b) :: totalI, totalJ, totalF
      character(len=1)  :: parity
      real(kind=dp)     :: energy
      real(kind=dp), dimension(:), pointer :: eigenvector
   ...
   !
   type, public :: asf_hfs_basis
      integer         :: noasf_ijf       ! Number of considered IJF-coupled ASF.
      integer         :: nocsf, nwshells, number_of_electrons, noasf
      real(kind=dp)   :: average_energy  ! Averaged energy of this set of ASF.
      type(as_hfs_function), dimension(:), pointer :: asf
   end type asf_hfs_basis
   !
...
contains
   ...
   !--------------------------------------------------------------------
   ! Appends all CSF with symmetries totalJ and parity from the 
   ! 'configuration scheme' csf_old to csf_new. It assumes and checkes
   ! that both configuration schemes have the 'same set and order'
   ! of subshells and the same number of electrons. The (optional)
   ! argument index(:) returns for the appended CSF their original
   ! index i in csf_old%csf(i).
   ! 
   ! Calls: 
   !--------------------------------------------------------------------
      !
      ...
      if (csf_old%nwshells > csf_new%nwshells) then
         print *, "csf_old%nwshells, csf_new%nwshells = ",              &
                   csf_old%nwshells, csf_new%nwshells
      ...
      do  i = 1,csf_old%nocsf
         if (csf_old%csf(i)%totalJ == totalJ  .and.  &
	     csf_old%csf(i)%parity == parity) n = n + 1
      end do
      !
	 ...
	 allocate( csf_new%csf(i)%occupation(1:nw), &
	           csf_new%csf(i)%seniority(1:nw),  &
	           csf_new%csf(i)%subshellJ(1:nw),  &
		   csf_new%csf(i)%subshellX(1:nw) )
	 ...
	 !
	 deallocate( csf_sav(i)%occupation, csf_sav(i)%seniority, &
	             csf_sav(i)%subshellJ,  csf_sav(i)%subshellX )
      end do
      !
      ...
      do  i = 1,csf_old%nocsf
         if (csf_old%csf(i)%totalJ == totalJ  .and.  &
	     csf_old%csf(i)%parity == parity) then
            ii = ii + 1
            csf_new%csf(ii)%totalJ = totalJ
            csf_new%csf(ii)%parity = parity
	    allocate( csf_new%csf(ii)%occupation(1:nw), &
	              csf_new%csf(ii)%seniority(1:nw),  &
	              csf_new%csf(ii)%subshellJ(1:nw),  &
		      csf_new%csf(ii)%subshellX(1:nw) )
   ...
   !--------------------------------------------------------------------
   ! Combines the two configuration schemes csf_a and csf_b into 
   ! the new scheme csf_c for which the memory is allocated inside of
   ! this procedure.
   ! All configuration schemes must represent the same number of electrons.
   !
   ...
   !--------------------------------------------------------------------
      ...
      if (key == "Case A") then
	    ...
	    allocate( csf_c%csf(i)%occupation(1:nw), &
                      csf_c%csf(i)%seniority(1:nw),  &
	              csf_c%csf(i)%subshellJ(1:nw),  &
	              csf_c%csf(i)%subshellX(1:nw) )
	    ...
	    allocate( csf_c%csf(ii)%occupation(1:nw), &
                      csf_c%csf(ii)%seniority(1:nw),  &
	              csf_c%csf(ii)%subshellJ(1:nw),  &
	              csf_c%csf(ii)%subshellX(1:nw) )
   ...
   !--------------------------------------------------------------------
   ! Copies and 'old' configuration scheme into a 'new' one.
   ! The memory for the 'new' scheme is allocated inside of this procedure.
   ! 
   ! Calls: 
   !--------------------------------------------------------------------
      !
   ...
   ! It is assumed that all the necessary arrays have been allocated properly
   ! before.
   ! 
   ! Calls: 
   !--------------------------------------------------------------------
   ...
   !--------------------------------------------------------------------
   ! Deallocates the storage of asf_set [type(asf_basis)] including
   ! the allocation of all underlying substructures.
   !
   ...
   !--------------------------------------------------------------------
   ! Deallocates the storage of asf_set [type(asf_det_basis)] including
   ! the allocation of all underlying substructures.
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Deallocates the storage of csf_set [type(csf_basis)] including
   ! the allocation of all underlying substructures.
   !--------------------------------------------------------------------
      !
      ...
      do  i = 1,csf_set%nocsf
         deallocate( csf_set%csf(i)%occupation,csf_set%csf(i)%seniority,&
                     csf_set%csf(i)%subshellJ, csf_set%csf(i)%subshellX,&
		     stat=ios)
      end do
      !
   ...
   !--------------------------------------------------------------------
   ! Return the seniority quantum number of a given antisymmetric 
   ! subshell state by looking up an appropriate table. 
   !--------------------------------------------------------------------
      !
    ...
    2 if (nu == -1) then
      ...
      return
      !
    1 print *, "Improper input for determining a subshell seniority "// &
               " qantum number or |kappa| > 11"
   ...
   !--------------------------------------------------------------------
   ! Reads in an parses the configuration symmetry list. A number of 
   ! checks are made to ensure correctness and consistency of the .csl 
   ! file.    
   !
   ! Calls: angular_momentum_j(), get_integer_from_string(), 
   !        get_kappa_from_name(), parse_intermediate_X(),
   !        parse_subshell_labels(), parse_subshell_J_and_seniority().
   !--------------------------------------------------------------------
      !
      type(csf_basis), intent(inout) :: csf_set
      !
      integer            :: i, j, k, kappa, nwcore, nwshells, nocsf, &
                            pqn, subsh
      logical            :: single_configuration
      character(len=800) :: recorda, recordb, recordc
    ...
    3 do  j = nwcore+1,nwshells
         ...
         do  i = 1,nwcore
            if (subshell_collect(i)%n     == pqn   .and.   &
                subshell_collect(i)%kappa == kappa)  then
      ...
      !
      ! Count the number of csl in the .csl file and allocate memory
      ! for them in the current configuration scheme
      nocsf = 0
      do  i = 1,1000000000
      ...
      !
    4 if (nocsf == 0) then
         ! Set values for a single closed-shell configuration which is
	 ! not set up by GRASP92
	 ...
	 single_configuration = .true.
      ...
      do  i = 1,nocsf
         allocate( csf_set%csf(i)%occupation(1:nwshells),  &
                   csf_set%csf(i)%seniority(1:nwshells),   &
                   csf_set%csf(i)%subshellJ(1:nwshells),   &
                   csf_set%csf(i)%subshellX(1:nwshells)  )
         !
         do  j = 1,nwcore
            csf_set%csf(i)%occupation(j) =  &
               angular_momentum_j(csf_set%subshell(j)%kappa) + 1
            ...
            do  k = 1,i-1
               do  j = 1,nwshells
                  if ( &
                   ...
                   csf_set%csf(k)%subshellX(j)  /= csf_set%csf(i)%subshellX(j)) &
                  then
                     goto 5
                  end if
               end do
    ...
    6 csf_set%number_of_electrons = &
         sum(csf_set%csf(1)%occupation(1:csf_set%nwshells))
      do  i = 1,nocsf
         if (sum(csf_set%csf(i)%occupation(1:csf_set%nwshells)) /= &
                 csf_set%number_of_electrons) then
   ...
   !
   !
   subroutine load_mix_file_grasp2k(asf_set,is_formatted,ierr)
   !--------------------------------------------------------------------
   ! Read in the eigenvectors of a number of ASF in a given ASF basis.
   ! It assumes and checks that the CSF basis of asf_set has been read 
   ! before. For is_formatted = .true., is assumes a formatted .mix file
   ! and an (unformatted) original GRASP92 .mix file otherwise.
   ! An error code ierr =/= 0 is returned if the routine was not successful.
   ...
   !--------------------------------------------------------------------
      ...
      type(asf_basis), intent(inout) :: asf_set
      !
      integer :: i, j, noasf, noasf_sofar, test_no_electrons, test_nocsf, &
                 test_nwshells
      ...
      if (test_no_electrons /= asf_set%csf_set%number_of_electrons   .or. &
          test_nocsf        /= asf_set%csf_set%nocsf                 .or. &
	 ...
	 ierr = 1
      ...
      if (is_formatted) then
         ...
         read (25,"(60(i5,a1))") (asf_set%asf(i)%totalJ,asf_set%asf(i)%parity,&
                                                           i = 1,noasf)
         read (25,"(61e26.19)") asf_set%average_energy, &
                               (eval(i),i = 1,noasf)
         do  i = 1,noasf
	 ...
	 !
       ...
       1 noasf_sofar = noasf_sofar + 60
         ...
         !
         do  j = 1,asf_set%csf_set%nocsf
            read (25,"(60e16.9)") (asf_set%asf(i)%eigenvector(j), &
                                     i=noasf_sofar+1,noasf)
         end do
         !
   ...
   10 continue
      ...
      close (25)
      !
   end subroutine load_mix_file_grasp2k
   !
   !
   subroutine parse_intermediate_X(record,ncsf,csf_set)
   !--------------------------------------------------------------------
   ! Parses the intermediate and total angular momenta and parity on 
   ! the character string record. These are the angular momenta of the
   ! CSF ncsf in the configuration scheme csf_set.
   !
   ...
   !--------------------------------------------------------------------
      ...
      logical :: open
      !
      ! Get total angular momentum and parity by looking 'backward' on the
      ! string
      j = scan(record,"+-",.true.)
      ...
      !
      !x print *, "parse_intermediate_X: ncsf,record = ",ncsf,record
      open = .false.
      do j = csf_set%nwcore+1,csf_set%nwshells
         ...
         if (csf_set%csf(ncsf)%occupation(j) == 0) then
            ...
            if (j > 1) then
               csf_set%csf(ncsf)%subshellX(j) = &
                  csf_set%csf(ncsf)%subshellX(j-1)
            else
            ...
            csf_set%csf(ncsf)%subshellX(j) =  &
                  ...
                  angular_momentum_j(csf_set%subshell(j)%kappa)+1) then
            ! For the second, ... open shell (with subshell-j /= 0) there
            ! must be a corresponding X value
            if (record(12:12) == "/") then
               csf_set%csf(ncsf)%subshellX(j) = &
                  get_dinteger_from_string(record(10:13))
            else if (record(11:11) == "/") then
               csf_set%csf(ncsf)%subshellX(j) = &
                  get_dinteger_from_string(record(9:12))
            else if (record(13:13) == "+"  .or.  record(13:13) == "-") then
               csf_set%csf(ncsf)%subshellX(j) = &
                  get_dinteger_from_string(record(10:12))
            else if (record(12:12) == "+"  .or.  record(12:12) == "-") then
               csf_set%csf(ncsf)%subshellX(j) = &
                  get_dinteger_from_string(record(10:11))
            else if (record(11:11) == "+"  .or.  record(11:11) == "-") then
               csf_set%csf(ncsf)%subshellX(j) = &
                  get_dinteger_from_string(record(10:10))
            else 
               csf_set%csf(ncsf)%subshellX(j) = &
                  get_dinteger_from_string(record(10:12))
            end if
   ...
   !--------------------------------------------------------------------
   ! Parses the subshell angular momenta J_sub for a given CSF ncsf
   ! on the character string record.
   !
   ! Calls: angular_momentum_j(), get_dinteger_from_string().
   !--------------------------------------------------------------------
      !
      ...
      do  j = csf_set%nwcore+1,csf_set%nwshells
         ...
         if (csf_set%csf(ncsf)%occupation(j) == 0) then
            !
                  ...
                  angular_momentum_j(csf_set%subshell(j)%kappa)+1) then
            csf_set%csf(ncsf)%subshellJ(j) = &
                  get_dinteger_from_string(record(6:9))
         end if
         !
         occ   = csf_set%csf(ncsf)%occupation(j)
         J2sub = csf_set%csf(ncsf)%subshellJ(j)
         if (angular_momentum_j(csf_set%subshell(j)%kappa) == 7   .and.  &
             occ == 4                                             .and.  &
            ...
            csf_set%csf(ncsf)%seniority(j) =  &
               get_integer_from_string(record(pos_semicolon-2:pos_semicolon-1))
         else
            csf_set%csf(ncsf)%seniority(j) = &
               get_subshell_seniority(csf_set%subshell(j)%kappa,occ,J2sub)
         end if
         record = record(10:256)
      end do
      !
   ...
   !--------------------------------------------------------------------
   ! Parses the subshell labels and the occupation of the (open) subshells
   ! as decribed on the character string record. These are the labels and
   ! the occupation of the CSF ncsf in the configuration scheme 
   ! csf_set.
   !
   ...
   !--------------------------------------------------------------------
      ...
      do  j = csf_set%nwcore+1,csf_set%nwshells
         ...
         if (csf_set%subshell(j)%n == get_integer_from_string(record(1:3)) &
            .and. &
             csf_set%subshell(j)%kappa == get_kappa_from_name(record(4:5)) &
             ) then
            csf_set%csf(ncsf)%occupation(j) = &
               get_integer_from_string(" "//record(7:8))
         else
            cycle
         end if
         record = record(10:256)
   ...
   !--------------------------------------------------------------------
   ! Prints all information about the CSF scheme csf_set in a neat 
   ! format on stream.
   ! 
   ! Calls: 
   !--------------------------------------------------------------------
      ...
      !
      write(stream,*) " "
      write(stream,*) "The current configuration scheme with ",csf_set%nocsf, &
                      " CSF is defined as follows:"
      write(stream,*) " "
      ...
      write(stream,1) (csf_set%subshell(i)%n,csf_set%subshell(i)%kappa, &
                                                    i=1,csf_set%nwshells)
      do  i = 1,csf_set%nocsf
         write(stream,*) " "
         write(stream,*) " CSF ",i,":   (",csf_set%csf(i)%totalJ,"/2", &
                         csf_set%csf(i)%parity,")"
   ...
   !--------------------------------------------------------------------
   ! Reads or writes a type(asf_det_basis) data structure from or to
   ! a file on stream.
   !--------------------------------------------------------------------
      !
      integer, intent(in)                :: stream
	 ...
	 call readwrite_det_basis(stream,read_from,asf%det_set)
      ...
      do  i = 1,asf%noasf
         if (read_from) then
	    read(stream,*) asf%asf(i)%level_No, asf%asf(i)%totalJ, &
	                   asf%asf(i)%parity,   asf%asf(i)%energy
	    allocate( asf%asf(i)%eigenvector(1:asf%det_set%nodmax) )
            do  j = 1,asf%det_set%nodmax
               read(stream,*) asf%asf(i)%eigenvector(j)
            end do
	 else
	    write(stream,*) asf%asf(i)%level_No, asf%asf(i)%totalJ, &
	                    asf%asf(i)%parity,   asf%asf(i)%energy
            do  j = 1,asf%det_set%nodmax
   ...
   !
   !
   subroutine set_configuration_scheme(csf_old,csf_new,pqn,kappa,  &
                       totalJ_old,parity_old,totalJ,parity,append,index)
   !--------------------------------------------------------------------
   ! Set up a (relativistic) 'configuration scheme' in arrays which are
   ! appropriate for calls to other procedures including those for
   ! angular coefficients. It 'copies' all CSF with totalJ_old and
   ! parity_old from csf_old [of type(csf_basis)] and extends these CSF
   ! by a (pqn,kappa)-subshell so that they become CSF with symmetries of
   ! totalJ and parity. If the (optional) argument append is present
   ! and append = .true. then these CSF are 'added' to a previously
   ! established CSF basis; it is assumed that these CSF fit together
   ! by their number_of_electrons, number of subshells, ...
   ! A test is made only after the 'new' CSF have been appended. 
   ! 
   ! Calls: 
   !--------------------------------------------------------------------
      !
	  ...
	  (ipc * ((-1)**l) == -1  .and.  parity == "+")  .or. &
         ...
         print *, "totalJ_old,parity_old,kappa,totalJ,parity = ", &
                   totalJ_old,parity_old,kappa,totalJ,parity
      ...
      nocsf_add = 0
      do  i = 1,csf_old%nocsf
         if (csf_old%csf(i)%totalJ == totalJ_old   .and.  &
             csf_old%csf(i)%parity == parity_old)  then
      ...
      if (present(append)   .and.   append) then
         ...
         do  i = 1,csf_new%nwshells
            if (csf_new%subshell(i)%n == pqn  .and.   &
                csf_new%subshell(i)%kappa == kappa) then
         ...
         allocate( csf_new%subshell(1:csf_new%nwshells), &
                   csf_new%csf(1:nocsf_add) )
         csf_new%subshell(1:nw)             = csf_old%subshell(1:nw)
         ...
         do  i = 1,csf_old%nocsf
	       ...
	       allocate( csf_new%csf(na)%occupation(1:nw+1), &
	                 csf_new%csf(na)%seniority(1:nw+1),  &
	                 csf_new%csf(na)%subshellJ(1:nw+1),  &
	                 csf_new%csf(na)%subshellX(1:nw+1) )
               ...
               csf_new%csf(na)%seniority(nw+1) = &
                                              get_subshell_seniority(kappa,1,j)
