...
!***** July 2010 *****
!-----------------------------------------------------------------------
! This module controls the computation of `angular coefficients' within the
! RATIP environment. At the present, angular coefficients may be obtained
! for all scalar (and symmetric) two-particle interactions. They are sometimes 
! called 'pure angular coefficients' in order to distinguish them from
! earlier approaches in GRASP (and its derivatives) where part of the
! `physical interaction' were included into the angular integrals.
! The 'pure' angular coefficients generally appear in the following form:
!
!    <CSF_l | operator | CSF_r> = sum_{t} coeff_t (L,abcd) * X^L (abcd)
!
! where CSF is a standard (jj-coupled) configuration state function,
! t a summation index, and X^L(abcd) denotes the effective interaction 
! strength. This effective strength is the only part which depends on the
! (scalar) operator. Thus, the `pure' coefficients can be applied to any
! interaction.
!
! The computation of the angular coefficients is based on the quasi-spin
! concept and the reduced coefficients of fractional parentage (rcfp).
! For details, see G. Gaigalas et al., J. Phys. B30 (1997) 3747.
!-----------------------------------------------------------------------
   !
   use rabs_angular
   use rabs_constant
   ...
   public  :: anco_calculate_csf_matrix
                 ! Calculates the angular coefficients for a sub-matrix of 
                 ! a given CSF scheme.
   public  :: anco_calculate_csf_matrix_1p
                 ! Calculates the angular coefficients for a one-particle
                 ! operator A of given rank nu for a sub-matrix of a CSF
                 ! scheme.
   ...
   public  :: anco_calculate_csf_pair_1p
                 ! Calculates the angular coefficients for a one-particle
                 ! operator A of given rank nu for a given pair of a CSF.
   public  :: anco_case_1
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distribution  1  (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_case_2_to_5
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 2 - 5 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_case_6
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions  6 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_case_7_to_14
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 11 - 14 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_case_7_to_8
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 7 - 8 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_case_9_to_10
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 9 - 10 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   ...
   public  :: anco_case_15_to_18
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 15 - 18 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   ...
   public  :: anco_case_19_to_42
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 19 - 26 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   ...
   public  :: anco_case_27_to_34
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 27 - 34 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_case_35_to_42
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for distributions 35 - 42 (of Table 1 in 
                 ! G. Gaigalas et al., 1997 J. Phys. B: At. Mol. Opt. Phys, 
                 ! Vol 30 3747).
   public  :: anco_codeing
                 ! Returns subshell states as appropriate to call
                 ! routines from the module rabs_rcfp.
   public  :: anco_collect_input
                 ! Collect and proceeds all input for the xanco program.
   private :: anco_diagonal_angle_s
                 ! Calculates the angular coefficients of none-scalar
                 ! one-particle operator for a given pair r, s of CSF
                 ! for diagonal matrix elements with respect to configurations.
   private :: anco_diff_occ_2
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for non-diagonal matrix elements with diff_occ = 2.
   ...
   private :: anco_one_particle_diag
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for diagonal matrix elements with respect to 
                 ! configurations for one--particle non scalar operator.
   private :: anco_one_particle_off
                 ! Calculates the angular coefficients for a given pair r, s 
                 ! of CSF for off diagonal matrix elements with respect to 
                 ! configurations for one--particle non scalar operator
   public  :: anco_one_particle_scalart
                 ! Calculates the angular coefficients for a given pair r, s
                 ! for one--particle scalar operator.
   public  :: anco_open_vnu
                 ! Opens a .vnu V^nu (abcd) Angular Coefficient file for the 
                 ! ANCO program on stream 25.
   public  :: anco_print_results
                 ! Prints a summary of the calculation in a neat format to 
                 ! the .sum file.
   ...
   !
   type(anco_T_coeff), dimension(1000)          :: anco_T_list
   type(anco_V_coeff), dimension(2000)          :: anco_V_list
   !
   ...
   !
   ! Define global logical flags for the control of the ANCO program; the
   ! default values for some of these flags may be overwritten interactively 
   ! during input time.
...
contains
   ...
   !--------------------------------------------------------------------
      ...
      if (anco_pure_one_particle   .and.   anco_total_T > 0) then
         ...
         print *, " Total number of GRASP2K-like T coefficients     =  ", &
                                                               anco_total_T
      end if
      !
   ...
   !--------------------------------------------------------------------
      ...
      if (.not.yes) then
         ...
         print *, " Generate GRASP2K-like T coefficients for scalar"//&
                  " interactions ?"
         yes = get_yes_stream()
      ...
      if(.not. anco_one_particle) then
         ...
         if (.not.yes) then
            ...
            print *, " Generate GRASP2K-like d coefficients for scalar"//&
                     " interactions ?"
            yes = get_yes_stream()
            if (yes) anco_pure_one_particle  = .false.        
     1      print *, " Enter the rank of the tensor"
      ...
      if (.not.yes) then
         ...
         print *, " Generate GRASP2K-like V^k coefficients for scalar"//&
                  " interactions ?"
         yes = get_yes_stream()
