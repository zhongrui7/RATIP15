...
!***** July 2010 *****
!-----------------------------------------------------------------------
! This module contains a number of mathematical and related functions
! as they appear frequently in the computation of atomic properties and
! structures.
!-----------------------------------------------------------------------
   !
   use rabs_constant
   ...
   public  :: complete_contraction4 
                 ! Calculates the complete contraction of a 4-dimensional with 
                 ! four vectors.
   public  :: coul_cern_kabachnik
                 ! Calculates the non-relativistic coulomb function for a
		 ! free electron with given energy and orbital momentum.
   public  :: distance
                 ! Calculates the distance between two (3-dim) points, given
   ...
   public  :: factorial        
                 ! Returns the value n! for given integer n.
   public  :: factorial_dp        
                 ! Returns the approximate value n! for given integer n 
                 ! as real(kind=dp).
   ...
   public  :: is_triangle
                 ! Returns .true. if three integers fulfill the triangular
                 ! condition and .false. otherwise.       
   public  :: legendre_associated
                 ! Returns the (complex) value of the associated Legendre 
		 ! function P_n^m (z) for complex argument.      
   public  :: legendre_polynomial
                 ! Returns the (complex) value of the Legendre polynomial
		 ! P_n (z) for complex argument. 
   ...
   public  :: set_beta_gamma_arrays 
                 ! Initializes array for beta_func(), gamma_func(), and 
                 ! incomplete_beta_func().
   public  :: set_gauss_zeros       
                 ! Returns the zeros and weights for a Gauss-Legendre 
                 ! integration.
   private :: set_gauss_zeros_aux  
                 ! Auxiliarity routine for set_gauss_zeros().
   public  :: spherical_Bessel_jL   
                 ! Returns the value of the spherical Bessel function j_L(x) 
                 ! for real values of x.
   public  :: spherical_Legendre_Pmn      
                 ! Returns the value of the spherical (associated) Legendre 
                 ! function P^m_n(x) for real x.
   ...
   public  :: Whittaker_M           
                 ! Returns the value of the Whittaker function M_a,b(x) for 
                 ! real values of x.
   public  :: Whittaker_W           
                 ! Returns the value of the Whittaker function W_a,b(x) for 
                 ! real values of x.
   !
   !
...
contains
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the Beta(a,b) = Beta(a2/2,b2/2) function 
   ! for integer and half-integer arguments using the array 
   ...
   !--------------------------------------------------------------------
   ! Calculates the determinant of the n x n matrix; ndim is the dimension
   ! in the declaration of matrix.
   !--------------------------------------------------------------------
      !
      integer, intent(in)        :: n
   ...
   !--------------------------------------------------------------------
   ! Returns the value of a complete contraction of the 4-dimensional
   ! array wa with the vectors va, vc, vb, vd in this sequence of indices.
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Coulomb wavefunctions calculated at r = rho by the continued-fraction 
   ! method of steed.   minl,maxl are actual l-values
   ! See Barnett et al Comp. Phys. Commun. v.8 p.377 (1974).
   !--------------------------------------------------------------------
      !
      ...
      real(kind=dp) :: rho, eta, accur, step, pace, acc, r, xll1, eta2, &
                       turn, tf, f, tfp, fp, etar, rho2, pl, pmx, dk, del, &
		       d, h, p, q, ar, ai, br, bi, wi, dr, di, dq, t, g, gp, &
		       w, r3, h2, dp_x, etah, h2ll, s, rh2, rh
      !
      !
      PACE = STEP
      ACC = ACCUR
...
11    W = 0.0
   ...
   !--------------------------------------------------------------------
   ! Returns the "distance" between two 3-dimensional points; these
   ! points can be given in either "cartesian" or "spherical" 
   ! coordinates.
   !
   ...
   !--------------------------------------------------------------------
      ...
      if (coordinates(1:9) == "cartesian") then
         ...
         value = sqrt( (x2w-x1w)*(x2w-x1w) + (y2w-y1w)*(y2w-y1w) + &
                       (z2w-z1w)*(z2w-z1w) )
   ...
   !--------------------------------------------------------------------
   ! Returns in cgamma the complex Gamma function of the complex argument 
   ! arg.  
   ! Only real(cgamma) is nonzero if aimag(arg) is zero. 
   ! The arctan function required must return angles (in radians) in
   ! the range  [0,2*\pi).
   ! This routine was originally written by F A Parpia and has been
   ! adapted to the Fortran 90/95 standard.
   !--------------------------------------------------------------------
      !
      ...
      real(kind=dp), dimension(7), save ::                             &
         fn = (/   1.0_dp,   -1.0_dp,    1.0_dp,   -1.0_dp,    5.0_dp, &
                -691.0_dp,    7.0_dp  /),                              &
         fd = (/   6.0_dp,   30.0_dp,   42.0_dp,   30.0_dp,   66.0_dp, &
                2730.0_dp,    6.0_dp  /)
      !
      integer       :: i
      real(kind=dp) :: argum, argur, argur2, argui, argui2, clngr, clngi,   &
                       diff, fac, facneg, obasq, obasqi, obasqr,            &
      ...
      if (aimag(arg) == zero) then
         ...
         ! diff <= precis+precis, originally
            ...
            if (diff <= eps10*eps10) then
               ...
               !
               ! Otherwise use the reflection formula (Abramowitz and Stegun 
               ! 6.1.17) to ensure that the argument is suitable for Stirling's 
               ! formula
    ...
    4    argum = sqrt(argur*argur + argui2)
      ...
      !
      ! Now exponentiate the complex log gamma function to get the complex 
      ! gamma function
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the Gamma(a) = Gamma(a2/2) function for
   ! integer and half-integer arguments using the array 
   ...
   !--------------------------------------------------------------------
   ! Calculates the value Gamma(x) / a^x by calling the function 
   ! nag_nag_s14aaf() from the NAG library in order to compute Gamma function
   ...
   !--------------------------------------------------------------------
   ! Returns the location xloc and the corresponding value yval of either
   ! a local maximum or minimum by interpolation. The 'function' must 
   ! be contained in y(n) = f(x(n)) and must be either convex or concav
   ! according to the keyword. It is also assumed that the maximum/minimum
   ! is not located 'too close' of one of the boundaries x(1) and x(n).
   !--------------------------------------------------------------------
      !
   ...
   ! 2F_1 = F(a,b,c;x) for real parameter a, b, and c, and for real
   ! argument x. The hypergeometric function is calculated by its
   ! power expansion as given by I. S. Gradshteyn and I. M. Ryzhik,
   ! Tables of Integrals, Series, and Products (Academic Press, New
   ! York, London a.o. 1980).
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the Gauss' hypergeometric function
   ! 2F_1 = F(a,b,c;x) for real parameter a, and c, and for complex b and
   ! argument z. The hypergeometric function is calculated by its
   ! power expansion as given by I. S. Gradshteyn and I. M. Ryzhik,
   ! Tables of Integrals, Series, and Products (Academic Press, New
   ! York, London a.o. 1980).
   !--------------------------------------------------------------------
   ...
   !--------------------------------------------------------------------
      ...
      !!	  * hypergeometric_2F1_complex_b(c-a,one-a,c-a-b+one,one-one/z)
               ...
               abs(atan2(aimag(-z),real(one-z))) < Pi) then
         F1 = gamma_complex(c)*gamma_complex(c-a-b)                        &
               /(gamma_complex(c-a)*gamma_complex(c-b))                    &
               * hypergeometric_2F1_complex_b(a,b,a+b-c+one,one-z)
         F2 = (one-z)**(c-a-b) *                                           &
                 gamma_complex(c)*gamma_complex(a+b-c)                     &
               /(gamma_complex(a)*gamma_complex(b))                        &
               * hypergeometric_2F1_complex_b(c-a,c-b,c-a-b+one,one-z)
         ...
         value = gamma_complex(c)*gamma_complex(b-a)                       &
               /(gamma_complex(b)*gamma_complex(c-a)) * (-z)**(-a)         &
               * hypergeometric_2F1_complex_b(a,one-c+a,one-b+a,one/z)     &
               + gamma_complex(c)*gamma_complex(a-b)                       &
               /(gamma_complex(c-b)*gamma_complex(a)) * (-z)**(-b)         &
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the degenerate hypergeometric function
   ! 1F_1 = Phi(a,b;x) for real parameters a and b, and for real
   ! argument x. The degenerate hypergeometric function is calculated 
   ! by applying its power expansion as given by I. S. Gradshteyn and 
   ! I. M. Ryzhik, Tables of Integrals, Series, and Products 
   ! (Academic Press, New York, London a.o. 1980).
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the incomplete Beta(a,b,x) = Beta(a2/2,b2/2,x) 
   ! function for integer and half-integer arguments a, b by using the 
   ! array beta_frac_coeff(:,:) which needs to be initialized before.
   !--------------------------------------------------------------------
      !
      integer, intent(in)       :: a2, b2
   ...
   !--------------------------------------------------------------------
   ! Returns a simple estimate of the integral value  int func(x) dx  
   ! with an equidistant step size delta_x. points is the number of grid 
   ! points and func(i) contains the corresponding values of the 
   ! function.
   !--------------------------------------------------------------------
      !
   ...
   ! interp() of GRASP2K which originally was written by F A Parpia.
   !--------------------------------------------------------------------
      !
      ...
      !
      logical       :: set
      integer       :: ibest, ilirok, ildiag, ilothr, irow, k, lhi, &
                       llo, llr, locnxt, nrsthi, nrstlo
   ...
   !--------------------------------------------------------------------
   ! Returns the (complex) value of the Legendre function P_n^m (z) for
   ! complex argument z.
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the (complex) value of the Legendre polynomial P_n (z) for
   ! complex argument z.
   !--------------------------------------------------------------------
      !
      integer, intent(in)              :: n
   ...
   !--------------------------------------------------------------------
   ! Calculates the LU decomposition of the n x n matrix; ndim is the 
   ! dimension in the declaration of matrix. This routine has been adopted
   ! from an algorithmus as discussed in the 'Numerical Recipies'.
   ! The real, intent(out) parameter d = 1.0 or d = -1.0 denotes the number
   ! of permutations and may be used to calculate the determinant as
   ! d times the product of all diagonal matrix elements (at output time).
   !--------------------------------------------------------------------
      !
      integer, intent(in)        :: n
   ...
   !--------------------------------------------------------------------
   ! Returns the value of Pochhammer's symbol (z)_n for real arguments of
   ! x.
   !--------------------------------------------------------------------
      !
      integer, intent(in)       :: n
   ...
   !--------------------------------------------------------------------
   ! This subprogram allocates and initializes some arrays for the 
   ! calculation of the Beta function, the incomplete Beta function, as 
   ! well as the Gamma function.
   !--------------------------------------------------------------------
      !
      integer       :: a, b, m
      ...
      do  b = 1, ndbeta
         do  a = 1, ndbeta
            beta_store(a,b) = gamma_store(a) * gamma_store(b) / &
                              gamma_store(a+b)
         end do
      enddo
      !
      ! The array beta_frac_coeff(n,a,b) contains fractional coefficients
      ! for the calculation of the incomplete Beta(x,a,b) function with 
      ! integer and half-integer arguments a,b
      !
      do b = 1,ndbeta
         do  a = 1,ndbeta
            wa = a / two
            ...
            do  m = 1,(ndmx/2)
               wm   = m
               wa2m = wa + wm + wm
               beta_frac_coeff(m+m,a,b)   = wm * (wb - wm) /  &
                                            ((wa2m - one) * wa2m)
               beta_frac_coeff(m+m+1,a,b) = - (wa + wm) * (wa + wb + wm) / &
                                            (wa2m * (wa2m + one) )
            end do
         end do
      end do
   ...
   !--------------------------------------------------------------------
   ! This subprogram set n-point gauss zeros and weights for the 
   ! interval (ax,bx) into the arrays z(1:nx) and w(1:nx) respectively.
   !
   ...
   !--------------------------------------------------------------------
      ...
      if (n < 1) then
         ...
         if ( n <= 16  .or.  n == 20  .or.  n == 24  .or.  n == 32  .or. &
              n == 40  .or.  n == 48  .or.  n == 64  .or.  n == 80  .or. &
              n == 96 ) then  
            goto 2
         else
            print *, "set_gauss_zeros(): n has a non-permissible value of ",n 
   ...
   !--------------------------------------------------------------------
   ! This subprogram is an auxilarity routine for set_gauss_zeros to
   ! set n-point Gauss zeros and weights.
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the spherical Bessel function 
   ! j_L(x) = sqrt(pi/2x) J_(L+1/2) (x) for real values of x by calling
   ...
   !--------------------------------------------------------------------
	 ...
	 print *, "nu, zz = ",nu, zz
      ...
      case(2)
         if (x < 1.0e-2) then
            test = x*x/15.0_dp - x**4/210.0_dp + x**6/7560.0_dp - &
                   x**8/498960.0_dp
         else
      ...
      case(3)
         if (x < 9.0e-2) then
            ...
            test = cos(x)/x - 6.0_dp*sin(x)/(x*x) - 15.0_dp*cos(x)/(x**3) + &
                   15.0_dp*sin(x)/(x**4)
         end if
      case(4)
         if (x < 3.0e-1) then
            test = x**4/945.0_dp - x**6/20790.0_dp + x**8/1081080.0_dp - &
                   x**10/97297200.0_dp
         else
            test = sin(x)/x + 10.0_dp*cos(x)/(x*x) - 45.0_dp*sin(x)/(x**3) - &
                   105.0_dp*cos(x)/(x**4) + 105.0_dp*sin(x)/(x**5) 
         end if
      case(5)
         if (x < 8.0e-1) then
            test = x**5/10395.0_dp - x**7/270270.0_dp + x**9/16216200.0_dp - &
                   x**11/1654052400.0_dp
         else
            test = -cos(x)/x + 15.0_dp*sin(x)/(x*x) + 105.0_dp*cos(x)/(x**3) - &
                   420.0_dp*sin(x)/(x**4) - 945.0_dp*cos(x)/(x**5) + &
                   945.0_dp*sin(x)/(x**6)
         end if
      case default
      ...
      value = test
      !
      !!x print *, "Bessel, L, x, test = ", L, x, test
      !
      !! ! Compare results
      !! if (abs((value - test)/value) > 1.0e-6_dp) then
      !!    print *, "spherical_Bessel_jL(): L, x, nag, direct = ", &
      !!                                     L, x, value, test
      !! end if
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the spherical (associated) Legendre function 
   ! P^m_n(x) as given in terms of the Gauss' hypergeometric function
   ! F(a,b,c;x) for real parameters a, b, c and real values of x.
   !
   ...
   !--------------------------------------------------------------------
      ...
      !
      if (rabs_use_naglib) then
         value = ((1+x)/(1-x))**(m*half) / nag_s14aaf(1-m,ifail)  * &
                 hypergeometric_2F1(-n,n+1,1-m,half*(one-x))
      else if (rabs_use_stop) then
   ...
   !--------------------------------------------------------------------
   ! This subprogram transforms the cartesian coordinates (x,y,z) into
   ! cylindrical coordinates (rho,phi,z). It terminates with a proper
   ! message if r = zero.
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! This subprogram transforms the cartesian coordinates (x,y,z) into
   ! spherical coordinates (r,theta,phi). It terminates with a proper
   ! message if r = zero.
   !--------------------------------------------------------------------
      !
      real(kind=dp), intent(in)  :: x, y, z
   ...
   !--------------------------------------------------------------------
   ! This subprogram transforms the cylindrical coordinates (rho,phi,z) 
   ...
   !--------------------------------------------------------------------
   ! This subprogram transforms the spherical coordinates (r,theta,phi) 
   ! into cartesian coordinates (x,y,z). 
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the Whittaker function M_a,b(x) for real
   ! parameters a and b and for real argument of x. 
   !
   ...
   !--------------------------------------------------------------------
      ...
      real(kind=dp)              :: value
      !
      value = x**(b+half) * exp(-x*half) * &
              hypergeometric_Phi(b-a+half,b+b+1,x)
      !
   ...
   !--------------------------------------------------------------------
   ! Returns the value of the Whittaker function W_a,b(x) for real
   ! parameters a and b and for real argument of x. 
   !
   ...
   !--------------------------------------------------------------------
      ...
      !
      if (rabs_use_naglib) then
         value = nag_s14aaf(-b-b,ifail) / nag_s14aaf(half-b-a,ifail) * &
                 Whittaker_M(a,b,x)  +                                 &
                 nag_s14aaf( b+b,ifail) / nag_s14aaf(half+b-a,ifail) * &
                 Whittaker_M(a,-b,x)
      else if (rabs_use_stop) then
