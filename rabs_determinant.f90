...
!***** July 2010 *****
!-----------------------------------------------------------------------
! This module provides abstract data types and procedures to facilitate 
! the work with determinants in atomic structure calculations. 
! The two main data types define a single 'determinant' and a
! 'determinant basis' for which the occupation of the individual 
! determinants are packed into standard integer arrays. 
!-----------------------------------------------------------------------
   !
   use rabs_constant
   ...
   public  :: cofactor_1_of_overlap_matrix
                 ! Calculates under a logical mask the n x n minorants 
                 ! (cofactor D(k,l)) for a quadratic matrix of overlap-
                 ! integrals as given by two determinants.
   public  :: cofactor_2_of_overlap_matrix
                 ! Calculates under a logical mask the n x n x n x n minorants 
                 ! (cofactor D(k1,k2;l1,l2)) for a quadratic matrix of overlap-
                 ! integrals as given by two determinants.
   public  :: distribute_m_in_ncells
                 ! Returns all possible distributions of M particle in 
                 ! N different cells.
   public  :: pack_occupation_in_integer
                 ! Packs the occupation numbers 0 and 1 of a determinant 
                 ! as 'bits' into an array of standard integers.
   public  :: pack_occupation_in_integer_nr
                 ! Packs the occupation numbers 0 and 1 of a nonrelativistic
                 ! determinant as 'bits' into an array of standard integers.
   public  :: readwrite_det_basis
                 ! Reads or writes a type(det_basis) data structure
		 ! from or to a file.
   public  :: unpack_occupation_from_integer
                 ! Unpacks the occupation numbers 0 and 1 of a determinant 
                 ! as 'bits' from an array of standard integers.
   public  :: unpack_occupation_from_int_nr
                 ! Unpacks the occupation numbers 0 and 1 of a nonrelativistic
                 ! determinant as 'bits' from an array of standard integers.
   !
...
contains
   !
   subroutine cofactor_1_of_overlap_matrix(norb,overlap,cofactors_1, &
                                                        cofactors_1_mask)
   !--------------------------------------------------------------------
   ! Calculates under control of the logical cofactors_1_mask the 
   ! norb x norb minorants (cofactor D(k,l)) for a quadratic matrix 
   ! overlap of overlap-integrals according to the two determinants 
   ! orbital_f and orbital_i.
   !--------------------------------------------------------------------
      !
   ...
   !
   !
   subroutine cofactor_2_of_overlap_matrix(norb,overlap,cofactors_2, &
                                                        cofactors_2_mask)
   !--------------------------------------------------------------------
   ! Calculates under control of the logical cofactors_2_mask the 
   ! norb x norb x norb x norb minorants (cofactor D(k,l)) for a quadratic 
   ! matrix of overlap-integrals according to the two determinants 
   ! orbital_f and orbital_i.
   !--------------------------------------------------------------------
      !
      integer, intent(in)                                 :: norb
   ...
   !--------------------------------------------------------------------
   ! Returns all possible distributions of M particle in N different 
   ! cells. In this version, the value of M must be m <= 7.
   !--------------------------------------------------------------------
      !
      ...
      case(4)
         do  i1 = 1,n-3;   do  i2 = i1+1,n-2;   do  i3 = i2+1,n-1
         do  i4 = i3+1,n
            d = d + 1
            distribution(i1,d) = 1;   distribution(i2,d) = 1
            distribution(i3,d) = 1;   distribution(i4,d) = 1
         end do;   end do;   end do;   end do
      case(5)
         do  i1 = 1,n-4;     do  i2 = i1+1,n-3;   do  i3 = i2+1,n-2
         do  i4 = i3+1,n-1;  do  i5 = i4+1,n
            d = d + 1
            distribution(i1,d) = 1;   distribution(i2,d) = 1
            distribution(i3,d) = 1;   distribution(i4,d) = 1
            distribution(i5,d) = 1
      ...
      case(6)
         do  i1 = 1,n-5;     do  i2 = i1+1,n-4;   do  i3 = i2+1,n-3
         do  i4 = i3+1,n-2;  do  i5 = i4+1,n-1;   do  i6 = i5+1,n
            d = d + 1
            distribution(i1,d) = 1;   distribution(i2,d) = 1
            distribution(i3,d) = 1;   distribution(i4,d) = 1
      ...
      case(7)
         do  i1 = 1,n-6;     do  i2 = i1+1,n-5;   do  i3 = i2+1,n-4
         do  i4 = i3+1,n-3;  do  i5 = i4+1,n-2;   do  i6 = i5+1,n-1
         do  i7 = i6+1,n
            d = d + 1
            distribution(i1,d) = 1;   distribution(i2,d) = 1
            distribution(i3,d) = 1;   distribution(i4,d) = 1
   ...
   !--------------------------------------------------------------------
   ! Packs the occupation numbers 0 and 1 of a determinant as 'bits'
   ! into an array of standard integers. It assumes that the array
   ! determinant%occupation(1:) has properly been allocated before.
   !--------------------------------------------------------------------
      !
   ...
   !--------------------------------------------------------------------
   ! Packs the occupation numbers 0 and 1 of a non-relativistic determinant 
   ! as 'bits' into an array of standard integers. It assumes that the array
   ! determinant%occupation(1:) has properly been allocated before.
   !--------------------------------------------------------------------
      !
      integer, intent(in) :: noint, norb
   ...
   !--------------------------------------------------------------------
   ! Reads or writes a type(det_basis) data structure from or to
   ! a file on stream.
   !--------------------------------------------------------------------
      !
      ...
      !
      if (read_from) then
         read(stream,*) det_set%nod, det_set%nodmax, det_set%norbital, &
	                det_set%number_of_electrons, det_set%noint
	 !x print *, "nodmax = ",det_set%nodmax
	 allocate( det_set%orbital(1:det_set%norbital),   &
	           det_set%determinant(1:det_set%nodmax) )
      else
         write(stream,*) det_set%nod, det_set%nodmax, det_set%norbital, &
	                 det_set%number_of_electrons, det_set%noint 
      end if
      !
      do  i = 1,det_set%norbital
         if (read_from) then
	    read(stream,*)  det_set%orbital(i)%n, det_set%orbital(i)%kappa, &
	                    det_set%orbital(i)%mm
	 else
	    write(stream,*) det_set%orbital(i)%n, det_set%orbital(i)%kappa, &
	                    det_set%orbital(i)%mm
	 end if
      end do
      !
      do  i = 1,det_set%nodmax
         if (read_from) then
	    read(stream,*) det_set%determinant(i)%totalM, &
	                   det_set%determinant(i)%parity
            allocate( det_set%determinant(i)%occupation(1:det_set%noint) )
            read(stream,*) (det_set%determinant(i)%occupation(j), &
	                                           j=1,det_set%noint)
	 else
	    write(stream,*) det_set%determinant(i)%totalM, &
	                    det_set%determinant(i)%parity
            write(stream,*) (det_set%determinant(i)%occupation(j), &
	                                            j=1,det_set%noint)
	 end if
      end do
      !
   ...
   !--------------------------------------------------------------------
   ! Unpacks the occupation numbers 0 and 1 of the determinant determ
   ! which are given as 'bits' from an array of standard integers.
   !--------------------------------------------------------------------
      !
      integer, intent(in) :: norb
