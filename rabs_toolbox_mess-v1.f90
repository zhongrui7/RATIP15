...
!-----------------------------------------------------------------------
   ...
   !
   !
   integer, public :: xmessage_nb01 = 2
   character(len=100), dimension(2) :: xmessage_b01 = (/                                               &
      " ",    &
      " " /)
   !
   !
   ...
   character(len=100), dimension(6) :: xmessage_c09 = (/                                               &
      ...
      " " /)
   !
   integer, public :: xmessage_nc11 = 8
   character(len=100), dimension(8) :: xmessage_c11 = (/                                               &
      "Modify and overwrite the total energies in a .mix Mixing Coefficient File; this helps to",      &
      "adapt transition energies without changing the mixing coefficients or orbitals. The program",   &
      "supports three modes for modifying the total energies in the units selected by the user.",      &
      " ",                                                                                             &
      " (i)   Enter energy shifts explicitly for all levels of interest.",                             &                
      " (ii)  Enter the total energies explicitly for all levels of interest.",                        &                
      " (iii) Enter excitation energies relative to some (explicit) reference energy for all ...",     &                
      " " /)
   !
   integer, public :: xmessage_nc15 = 7
   character(len=100), dimension(7) :: xmessage_c15 = (/                                               &
      "For a given .csl list, this subprogram re-collects and combines radial orbitals from several",  &
      "(formatted) .out files into a new (formatted) .out file. This feature helps to deal with",      &
      "formatted orbitals files and in cases where ERWF does not work properly.",                      &
      " ",                                                                                             &
      "WARNING: No attempt is made at present to analyze the grid structure; the user has to make ",   &
      "-------- sure that the grid is the same in all orbital files.",                                 &
      " " /)
   !
   !
   !
   ...
   !
   !
   integer, public :: xmessage_nf01 = 3
   character(len=100), dimension(3) :: xmessage_f01 = (/                                               &
      "Calculate the one-electron multipole matrix elements for a given set of radial orbitals.",      &
      "Different definitions and phases are available for these matrix elements.",                     &
      " " /)
   !
   integer, public :: xmessage_nf02 = 3
   character(len=100), dimension(3) :: xmessage_f02 = (/                                               &
      "Calculate effective Coulomb interaction strengths for a given set of radial orbitals, i.e. ",   &
      "   X^L_Coulomb(na, kapa; nb, kapb; nc, kapc; nd, kapd).",                                       &
      " " /)
   !
   integer, public :: xmessage_nf03 = 3
   character(len=100), dimension(3) :: xmessage_f03 = (/                                               &
      "Calculate (frequency-independent) effective Breit interaction strengths for a given set ",      &
      "of radial orbitals, i.e. X^L_Breit0(na, kapa; nb, kapb; nc, kapc; nd, kapd).",                  &
      " " /)
   !
   !
   !
   integer, public :: xmessage_ng01 = 3
   character(len=100), dimension(3) :: xmessage_g01 = (/                                               &
      "Provides alternative (but equivalent) information about some given transition amplitude or ",   &
      "rate. It requests the amplitude or rate as well as the energy.",                                &
      " " /)
   !
   integer, public :: xmessage_ng04 = 5
   character(len=100), dimension(5) :: xmessage_g04 = (/                                               &
      "Calculates branching ratios from the output of the  REOS/EINSTEIN  programs. For this,",        &
      "The program reads one or several  .trn  transition data files which have been generated",       &
      "in independent runs of the REOS or EINSTEIN program. Branching ratios refer to a common",       &
      "'initial' level which is determined due to its total energy.",                                  &
      " " /)
   !
   integer, public :: xmessage_ng06 = 6
   character(len=100), dimension(6) :: xmessage_g06 = (/                                               &
      "Generates transition probability tables from the output of the  REOS/EINSTEIN  programs.",      &
      "For this, the program reads one or several  .trn  transition data files which have been",       &
      "generated before in independent runs of the REOS or EINSTEIN program. The main purpose is",     &
      "to combine the results from different runs and to provide a larger flexibility in the",         &
      "tabulation of the data. ",                                                                      &
      " " /)
   !
   !
   !
   integer, public :: xmessage_nh01 = 2
   character(len=100), dimension(2) :: xmessage_h01 = (/                                               &
      " ",    &
      " " /)
   !
   !
   !
   integer, public :: xmessage_ni01 = 5
   character(len=100), dimension(5) :: xmessage_i01 = (/                                               &
      "Calculate the sequential two-photon double-ionization cross sections and angular parameters.",  &
      "For this, the program expects two .trn PHOTO transition files from the first and second step ", &
      "in order to generate the density matrix, alignment and second-step angular distribution ",      &
      "parameters. ",                                                                                  &
      " " /)
   !
   integer, public :: xmessage_ni02 = 8
   character(len=100), dimension(8) :: xmessage_i02 = (/                                               &
      "Calculate the angle-angle correlation function for the two electrons emitted in the ",          &
      "two-photon double-ionization of atoms and ions. It presently assumes the ionization scheme ",   &
      "   n_0 --> [n_i, n_i', n_i'', ...] --> n_f ",                                                   &
      "where the ionization may proceed coherently via two or several intermediate states.",           &
      "For this, the program expects two .trn PHOTO transition files from the first and second step ", &
      "in order to generate the density matrix, alignment and angle-angle correlation function. ",     &
      "The specific definition of the levels and angles need to be given internally by modifying ",    &
      "the source code." /)
   !
   !
   !
   integer, public :: xmessage_nj01 = 3
   character(len=100), dimension(3) :: xmessage_j01 = (/                                               &
      "Determines for a given transition the isotope shift in terms of the M- und F-values.  ",        &
      "These values are calculated in turn for one or several groups of 3 isotopes; ",                 &
      "the final result is the average value over all the possible groups." /)
   !
   integer, public :: xmessage_nj07 = 4
   character(len=100), dimension(4) :: xmessage_j07 = (/                                               &
      "Calculates the HFS splitting Delta_nu (in Hz) for given A and B values following the two-",     &
      "parameter HFS formula (H. Kopfermann, Nuclear Moments, 1958). ",                                &
      "The splitting is displayed either for a single J^P level or a given J-J' transition. ",         &
      " " /)
   !
   integer, public :: xmessage_nj08 = 7
   character(len=100), dimension(7) :: xmessage_j08 = (/                                               &
      "Calculates HFS quenched transition rates and lifetimes from the IJF-coupled ASF of the ",       &  
      "initial and final atomic states. The program assumes two  hfs.mix files for these ASF as ",     &
      "generated by the HFS program as well as a .trn amplitude file from either the EINSTEIN or ",    &
      "REOS program. Although no information about the corresponding csl.inp and relci.mix files ",    &
      "is requested, some tests are made that the given data files are consistent to each other. ",    &
      "Results are tabulated in a form similar to the EINSTEIN program.",                              &
      " " /)
   !
   !
   !
   ...
   character(len=100), dimension(5) :: xmessage_m05 = (/                                               &
      ...
      " " /)
   !
   integer, public :: xmessage_nm06 = 2
   character(len=100), dimension(2) :: xmessage_m06 = (/                                               &
      " ",    &
      " " /)
   !
   integer, public :: xmessage_nm07 = 6
   character(len=100), dimension(6) :: xmessage_m07 = (/                                               &
      "Generates a set of approximate non-relativistic orbitals from a given (relativistic)",          &
      "GRASP2K  .rwf  orbital file. To this end, only the large components are considered and",        &
      "renormalized on a new grid which has to be specified. For other than s-type orbitals,",         &
      "only those orbitals are considered for which both relativistic orbital functions",              &
      "with j = l+1/2 and j = l-1/2 are given explcitely.",                                            &
      " " /)
   !
   integer, public :: xmessage_nm08 = 6
   character(len=100), dimension(6) :: xmessage_m08 = (/                                               &
      "Expand a given set of GRASP2K atomic state functions (ASF) into a 'complete set' of",           &
      "nonrelativistic determinants. This determinant set, which is built on (n,l,m_l,m_s)",           &
      "spin-orbitals includes determinants with all possible M_L and M_S total projections of",        &
      "the total orbital angular momentum, L, and total spin, S. Such 'complete sets' are, ",          &
      "for instance, required for collision calculations.",                                            & 
      " " /)
   !
   !
   !
   ...
   !
   !
   integer, public :: xmessage_np01 = 2
   character(len=100), dimension(2) :: xmessage_p01 = (/                                               &
      "Calculate ... ",                                                                                &
      "          ... " /)
   !
   integer, public :: xmessage_np02 = 4
   character(len=100), dimension(4) :: xmessage_p02 = (/                                               &
      "Calculate the EDM enhancement factors d^pss and d^tpt for some given set of many-electron",     &
      "interaction amplitudes from the SCHIFF module; this includes the summation over a set of",      &
      "intermediate states which need to be specified together with their energy with regard to the",  &
      "reference states |0>." /)
   !  
   !
   !
