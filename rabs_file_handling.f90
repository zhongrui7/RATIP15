...
!***** July 2010 *****
!-----------------------------------------------------------------------
! This module maintains most of the procedures to read in and to write
! out information to/from the (GRASP92) interface files. Attempts has been
! made to formalize the inquiry of file names and the reading of the data.
!-----------------------------------------------------------------------
   !
   use rabs_constant
   use rabs_csl
   ...
   public  :: file_get_csl_list
                 ...
                 ! CSF into the internal arrays.
   public  :: file_get_eigenvectors_unformatd
                 ! Reads in the eigenvectors of a number of ASF from a standard
                 ! GRASP-92 .mix mixing coefficient file.
   public  :: file_get_eigenvectors_formatted
                 ! Reads in the eigenvectors of a number of ASF from a
                 ! formatted GRASP-92 .mix mixing coefficient file.
   public  :: file_get_isodat_grasp2k    
                 ! Open, check, load data from and close the  .iso  file.
   public  :: file_get_mix
		 ! Opens a .mix GRASP92 mixing file and reads in the levels
	         ! and mixing coefficients for the given states.
   public  :: file_get_rwf
                 ! Opens a .out file and reads in the radial orbitals for 
                 ! the bound states.
   public  :: file_get_rwf_det
                 ! Opens a .out file and reads in the radial orbitals for 
                 ! the bound states within a determinant basis.
   public  :: file_get_xpn
                 ! Opens a .xpn file and reads in the determinant expansion for 
                 ! some bound states.
   private :: file_load_xpn
		 ! Loads all necessary input from an .xpn file into an
		 ! ASF determinant basis structure.
   public  :: file_read_isodat_grasp2k  
                 ! Reads the nuclear parameters from a grasp2k .iso file.
   public  :: file_open            
   ...
   !
   ! Define global logical flags for the control of the file_handling module;
   ! the default values for these flags may be overwritten interactively 
   ! during input time
...
contains
   ...
   ! loads the list of CSF into the internal arrays. This file is always
   ! attached to stream 21.
   !
   ...
   !--------------------------------------------------------------------
    ...
    1 print *, wstring
      ...
      !
      call save_input(csl_list_file,.false.)
      !
   ...
   !--------------------------------------------------------------------
   ! Read in the eigenvectors of a number of ASF in a given CSF basis.
   ! Open, check, load data from and close the  standard .mix  file 
   ! from GRASP-92. This file is always attached to stream 25.
   !--------------------------------------------------------------------
      !
    ...
    2    print *, "Enter the name of the GRASP92 mixing coefficient file:"
      ...
      if (test_no_electrons /= asf_set%csf_set%number_of_electrons   .or. &
         ...
         print *, "This mixing coefficients file is not appropriate to "// &
                  "the CSF basis"
   ...
   !--------------------------------------------------------------------
   ! Read in the eigenvectors of a number of ASF in a given CSF basis.
   ! Open, check, load data from and close the  formatted .mix  file 
   ! a generated from an unformatted GRASP92 .mix file. 
   ...
   !
   !
   subroutine file_get_isodat_grasp2k()
   !--------------------------------------------------------------------
   ! Open, check, load data from and close the  .iso  file. This file 
   ! is always attached to stream  22.
   !--------------------------------------------------------------------
      !
      implicit none
      integer            :: ierr, ios
      ...
      file_default = "grasp2k.iso"
      file_format  = "formatted"
      file_status  = "old"
      !
      ! Look for  grasp2k.iso
      inquire (file = file_default, exist = found)
      !
    1 if (found) then
         !
         ! File  grasp2k.iso  exists; ascertain that it is to be used;
         ! if it is not to be used, determine another filename
         print *, "file  grasp2k.iso  found; enter another file name "// &
                  "if this file"
         print *, " is not to be used as the isotope data file;"
         print *, " <cr> otherwise:"
         read (*, "(a)") file_name
         ...
         !
         ! File  grasp2k.iso  does not exist; determine the name of the
         ! .iso  file
         print *, "Enter the name of the isotope data file:"
         read (*, "(a)") file_name
         if (len_trim (file_name) == 0) then
            found = .false.
            goto 1
      ...
      !
      call save_input(file_name,.false.)
      !
      ...
      if ((ios .ne. 0)  .or.  (record(1:14) .ne. 'Atomic number:')) then
         print *, "not an isotope data file;"
         close (22)
         found = .false.
         goto 1
      endif
      ...
      call file_read_isodat_grasp2k()
      !
      ! close the  .iso  file
      close (22)
      !
   end subroutine file_get_isodat_grasp2k
   !
   !
   subroutine file_get_mix(wstring,asf_set)
   !--------------------------------------------------------------------
   ! Opens a final-state .mix Mixing Coefficient File from GRASP92 and 
   ! reads in all required levels and mixing coefficients.
   !
   ...
   !--------------------------------------------------------------------
    ...
    3 call file_open(25,mix_file,"formatted  ","old",ierr)
	 ...
	 if (ierr /= 0) then
      ...
      !
      call save_input(mix_file,.false.)
      !
      ! Close the  .mix  file
    4 close (25)
      !
   ...
   !
   !
   subroutine file_get_rwf(wstring,asf_bound,wave_bound,is_allocated)
   !--------------------------------------------------------------------
   ! Opens a bound-state .rwf Radial WaveFunction File from GRASP92 and 
   ! reads in all required orbital wave functions. 
   !
   ! Calls: load_rwf_file_grasp2k(), file_open().
   !--------------------------------------------------------------------
      !
      character(len=*), intent(in)         :: wstring
      type(asf_basis), intent(inout)       :: asf_bound
      type(grasp2k_orbital), intent(inout) :: wave_bound
      logical, intent(in)                  :: is_allocated
      !
      integer            :: iorb, ierr, ios
      character(len=31)  :: record
      character(len=256) :: file_rwf
      !
    1 print *, wstring
      read (*,"(a)") file_rwf
      ...
      if (.not.is_allocated) then
         ...
         do  iorb = 1,asf_bound%csf_set%nwshells
            ...
            wave_bound%rwf(iorb)%orbital%kappa = &
	                               asf_bound%csf_set%subshell(iorb)%kappa
      ...
      call load_rwf_file_grasp2k(wave_bound,file_use_formatted_rwf,ierr)
      if (ierr /= 0) then
         goto 1
      end if
      !
   ...
   !--------------------------------------------------------------------
   ! Opens a .rwf Radial WaveFunction File from GRASP92 and reads in 
   ! all required orbital wave functions. 
   !
   ! Calls: load_rwf_file_grasp2k(), file_open().
   !--------------------------------------------------------------------
      !
      ...
      character(len=256) :: file_rwf
      !
    1 print *, "Enter the name of the final-state Radial WaveFunction File:"
      read (*,"(a)") file_rwf
      if (len(trim(file_rwf)) == 0) goto 1
      !
      call file_open(21,file_rwf,"unformatted","old",ierr)
   ...
   !--------------------------------------------------------------------
   ! Opens a .xpn file for some (bound) states and reads in all necessary 
   ...
   !--------------------------------------------------------------------
      ...
      character(len=256) :: file_xpn
      !
      !!x 1 print *, "Enter the name of the initial-state CESD .xpn expansion file:"
    1 print *, wstring
      !!x print *, " This file must be in the new compact format."
      read (*,"(a)") file_xpn
   ...
   !--------------------------------------------------------------------
   ! Loads all necessary input from a CESD .xpn output file on stream 21
   ! into a ASF determinant basis structure.
   !
   ...
   !--------------------------------------------------------------------
      ...
      type(asf_det_basis), intent(inout) :: asf_load
      !
      integer :: i, iorb,iorbtest, iblock, idet, idettest, imax, &
		 isu, iso, itest, nblock, nobit, noint, nn
      ...
      !
      ! First read the orbital reference list and then the list of determinants
      ! in terms of their occupation numbers with respect to the reference list
      !
	 ...
	 !
	 read (21, "(6(i4,1x,3i4,4x))") (iorbtest,  &
	    asf_load%det_set%orbital(i)%n,	    &
	    asf_load%det_set%orbital(i)%kappa,      &
	 ...
	 if (nobit <= 32) then
	    !!x print *, "idet, noint = ",idet, noint
	    read (21, "(i7,3x,i4,a1,2x,40(i12))") idettest, &
		asf_load%det_set%determinant(idet)%totalM,  &
		asf_load%det_set%determinant(idet)%parity,  &
	       (asf_load%det_set%determinant(idet)%occupation(i),i=1,noint)
	 else
	    read (21,"(i7,3x,i4,a1,2x,40(i24))") idettest,  &
		asf_load%det_set%determinant(idet)%totalM,  &
		asf_load%det_set%determinant(idet)%parity,  &
	       (asf_load%det_set%determinant(idet)%occupation(i),i=1,noint)
	 end if
	 !!x print *, "read D, idet = ",idet
	 if (rabs_use_stop   .and.   idet /= idettest) then
      ...
      call unpack_occupation_from_integer(				   &
			     asf_load%det_set%determinant(1),occupation,nn)
   ...
   !
   !
   subroutine file_read_isodat_grasp2k()
   !--------------------------------------------------------------------
   ! Reads the data from the  grasp2k .iso  file. 
   !--------------------------------------------------------------------
      !
      implicit none
      ...
      ! Atomic number and nuclear geometry
      ...
      print *, ' ... load complete;'
      !
   end subroutine file_read_isodat_grasp2k
   !
   !
   subroutine file_open(nfile,file_name,file_format,file_status,ierr)
   !--------------------------------------------------------------------
   ! Issues open for file with unit number nfile, name  file_name, 
   ! format  file_format, status file_status.  If this is successful 
   ! the head is positioned to the beginning of the file and ierr is 0; 
   ! otherwise ierr is set to 1. This subroutine was originally written
   ! by F A Parpia and adapted to rabs.
   !--------------------------------------------------------------------
      !
      ...
      !
      open (nfile, file = file_name, form = file_format,  &
                 status = file_status, iostat = ios)
      !
      if (ios == 0) then
         rewind (nfile)
         ierr = 0
         ...
         print *, "file_open(): error opening file ",file_name(1:loc), &
                  " as ",file_status,";"
         ierr = 1
      end if
      !
   ...
   !--------------------------------------------------------------------
      ...
      !
    1 print *, "Enter the name of the .mix  mixing coefficient file "// &
      ...
      write (25,"(60i6)") (asf_set%asf(levels(i))%level_No,                 &
                            i=1,noasf)
      write (25,"(60(i5,a1))") (asf_set%asf(levels(i))%totalJ,              &
                                asf_set%asf(levels(i))%parity,              &
                                i=1,noasf)
      !
      write (25,"(61e26.19)") asf_set%average_energy,                       &
                              (asf_set%asf(levels(i))%energy,               &
			      i=1,noasf)
      do j = 1, asf_set%csf_set%nocsf
         write (25,"(60e16.9)") (asf_set%asf(levels(i))%eigenvector(j),     &
                                 i=1,noasf)
      end do
      !  
    ...
    3 noasf_sofar = noasf_sofar + 60
      ...
      write (25,"(60(i5,a1))") (asf_set%asf(levels(i))%totalJ,              &
				...
				i = noasf_sofar+1,noasf)
      !
      write (25,"(60e26.19)") (asf_set%asf(levels(i))%energy,               &
                              i = noasf_sofar+1,noasf)
      do  j = 1,asf_set%csf_set%nocsf
	 write (25,"(60e16.9)") (asf_set%asf(levels(i))%eigenvector(j),     &
				  i=noasf_sofar+1,noasf)
